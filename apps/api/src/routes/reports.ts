...
reportsRouter.get('/api/v1/reports/data-guru', requireAuth, requireRole(['ADMIN', 'TU', 'KEPSEK']), async (_req, res, next) => {
  try {
...
reportsRouter.get('/api/v1/reports/data-siswa', requireAuth, requireRole(['ADMIN', 'TU', 'KEPSEK', 'GURU']), async (_req, res, next) => {
  try {
...
reportsRouter.get('/api/v1/reports/ledger-nilai', requireAuth, requireRole(['ADMIN', 'KEPSEK', 'GURU']), async (req, res, next) => {
  try {
    const q = z
...
reportsRouter.get('/api/v1/reports/kekurangan-spp', requireAuth, requireRole(['ADMIN', 'TU', 'KEPSEK']), async (req, res, next) => {
  try {
